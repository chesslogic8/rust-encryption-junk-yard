...
const MAGIC: &[u8] = b"APQ1";  // Keep distinct if desired
const VERSION: u8 = 1;
const SALT_SIZE: usize = 32;
const NONCE_SIZE: usize = 16;
const KEY_SIZE: usize = 16;           // Reverted to 128-bit (crate limitation)
...
#[derive(Parser)]
#[command(author, version, about = "File encryptor using Ascon (160-bit PQ placeholder)")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn derive_key(password: &str, salt: &[u8; SALT_SIZE]) -> Result<Key> {
    let mut key = [0u8; KEY_SIZE];

    let params = argon2::ParamsBuilder::new()
        .m_cost(DEFAULT_MEMORY_KIB)
        .t_cost(DEFAULT_TIME)
        .p_cost(DEFAULT_PARALLEL)
        .build()
...
fn get_key(keyfile: Option<&Path>, prompt: bool) -> Result<Key> {
    if let Some(kf) = keyfile {
        ...
        Err(anyhow!("must provide --keyfile or use interactive password mode"))
    }
}
...
fn encrypt(args: EncryptArgs) -> Result<()> {
    ...
    pb.set_style(
        ProgressStyle::default_spinner()
            .template("{spinner} Reading input...")?
    );
    ...
    let mut key = get_key(args.keyfile.as_deref(), true)?;
    ...
    let out_path = args.output.unwrap_or_else(|| {
        ...
        args.input.with_file_name(format!("{}.asconpq", stem))
    });
    ...
    if args.shred && args.input.to_string_lossy() != "-" {
        shred_file(&args.input, plain.len())?;
    }
...
fn decrypt(args: DecryptArgs) -> Result<()> {
    ...
    if data.len() < 4 + 1 + SALT_SIZE + 4*3 + NONCE_SIZE + 1 {
    ...
    if data[pos] != VERSION {
        return Err(anyhow!("unsupported version: got {}, want {}", data[pos], VERSION));
    }
    pos += 1;
    ...
    let _mem     = u32::from_le_bytes(data[pos..pos + 4].try_into()?); pos += 4;
    let _time    = u32::from_le_bytes(data[pos..pos + 4].try_into()?); pos += 4;
    let _parallel = u32::from_le_bytes(data[pos..pos + 4].try_into()?); pos += 4;
    ...
    let plain = cipher.decrypt(AsconAead128Nonce::from_slice(&nonce), payload)
        .map_err(|e| anyhow!("decryption failed (likely wrong key, tampered file, or corrupted data): {}", e))?;
